...
type DeviceView struct {
	...
	// the device's other controls (select/number/switch siblings)
	Extras []ExtraView
}

// ExtraView is one sibling control of a device tile; serialised into the
// tile's data-extras for the popover, its state kept fresh by /live.json.
...
func deviceTileAttrs(d DeviceView) string {
	...
	if len(d.PresetModes) > 0 {
		modes, _ := json.Marshal(d.PresetModes)
		fmt.Fprintf(&b, ` data-preset-modes="%s" data-preset-mode="%s"`, html.EscapeString(string(modes)), html.EscapeString(d.PresetMode))
	}
	if len(d.Extras) > 0 {
		extras, _ := json.Marshal(d.Extras)
		fmt.Fprintf(&b, ` data-extras="%s"`, html.EscapeString(string(extras)))
	}
	return b.String()
}
